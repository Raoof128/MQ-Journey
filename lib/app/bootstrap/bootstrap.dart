...
Future<void> bootstrap(Widget Function() appBuilder) async {
  ...
  await runZonedGuarded(
    () async {
      ...
      // Run Firebase and Supabase initialisation in parallel — neither depends
      // on the other, and running them sequentially would add their timeouts
      // together (up to 15 s).  Parallel execution caps the wait at the slower
      // of the two (10 s for Supabase).
      await Future.wait([
        ...
        // supabase_flutter reads any cached session from the Keychain, then
        // makes a network call to refresh an expired access token before
        // returning. Without a cable/debugger the device uses its own network
        // path and that HTTP request can stall indefinitely on a weak or
        // unavailable connection. iOS's watchdog (~20 s) would eventually kill
        // the process, but by then the native red launch screen has been
        // visible for the user for the entire wait — because runApp() is never
        // reached.
        //
        // The 10-second timeout is safe: supabase_flutter assigns
        // Supabase._instance synchronously before any async session work, so
        // Supabase.instance is usable even when we time out. The GoRouter
        // redirect treats a missing session as unauthenticated → /auth/login,
      ...
      // and a top-level ErrorBoundary to catch rendering exceptions. Called
      // as early as possible — remaining non-critical initialisation is
      // deferred to the idle callback so the user sees the login / splash page
      // sooner rather than staring at the native launch screen.
      runApp(ProviderScope(child: ErrorBoundary(child: appBuilder())));

      // Deferred: ObjectBox / FMTC initialisation is non-critical — the app
      // functions perfectly with online-only map tiles if it fails. Running
      // it after runApp() means the first Flutter frame is drawn before the
      // FFI native library is loaded. The 8-second timeout in
      // initializeBackend() ensures a corrupted or slow ObjectBox store
      // cannot delay tiles past a tolerable bound. Any native-level crash
      // inside ObjectBox that bypasses Dart's try/catch still lands in the
      // zone error handler set up above.
      try {
        await const OfflineMapsService().initializeBackend();
      } catch (error, stackTrace) {
        AppLogger.warning(
          'Offline-maps backend initialisation skipped. '
