...
import 'package:mq_journey/features/scan/domain/fakes/fake_location_content.dart';
...
final trailRepositoryProvider = Provider<TrailRepository>((ref) => TrailRepository());
final indoorRepositoryProvider = Provider<IndoorRepository>((ref) => IndoorRepository());
final buildingsRepositoryProvider =
    Provider<BuildingsRepository>((ref) => BuildingsRepository());
...
final indoorManifestProvider =
    FutureProvider.family<IndoorManifest?, String>((ref, buildingId) {
  return ref.read(indoorRepositoryProvider).load(buildingId);
});

final locationContentProvider =
    Provider.family<LocationContent?, String>((ref, locationId) {
  return ref.watch(fakeLocationContentProvider(locationId));
});
...
final myDayApiProvider = Provider<FakeMyDayApi>((ref) => FakeMyDayApi());

final visitedStateProvider =
    StreamProvider.family<VisitedState, String>((ref, locationId) {
