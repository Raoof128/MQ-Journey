...
class _BuildingSearchSheetState extends ConsumerState<BuildingSearchSheet> {
  ...
  /// Single exit path for the sheet.
  ///
  /// Drops focus *before* popping so the keyboard retracts with the sheet
  /// rather than after it — otherwise the route is already gone when the IME
  /// animates out and the frame underneath visibly jumps.
  void _close([Building? result]) {
  ...
  Widget build(BuildContext context) {
    ...
    // Watch the whole AsyncValue, not just `.value`: while the registry is
    // still decoding, `.value` is null and the old code fell through to an
    // empty ListView, which is exactly the "blank list / stuck loading" the
    // sheet showed on a cold first open.
    final mapAsync = ref.watch(mapControllerProvider);
    final state = mapAsync.value;
    final results = state?.searchResults ?? const <Building>[];
    final query = _controller.text.trim();
    final isLoading = state == null && mapAsync.isLoading;
    final error = state == null ? mapAsync.error : null;
    ...
    // `showModalBottomSheet` does NOT apply keyboard insets (there is no
    // `viewInsets` handling anywhere in Flutter's bottom_sheet.dart), and this
    // sheet autofocuses its field. Without the padding below, the IME slides
    // up over a sheet that never shrinks: the list's last rows sit behind the
    // keyboard and the layout visibly lurches on first open. Padding the sheet
          ...
          child: Column(
            ...
            children: [
                  ...
                  children: [
                    Expanded(
                      child: TextField(
                        ...
                        // What you typed must outrank the hint: full-strength
                        // colour + w500 against a lighter, unweighted hint.
                        // Both sit on the sheet's SOLID surface, so unlike the
                        // map pill their contrast is predictable.
                        style: TextStyle(
                          color: isDark
                              ? Colors.white
              ...
              Expanded(
                child: isLoading
                    ? const Center(
                        key: Key('building-search-loading'),
                        child: Padding(
                          padding: EdgeInsets.all(MqSpacing.space6),
                          child: CircularProgressIndicator(),
                        ),
                      )
                    : error != null
                    ? Center(
                        key: const Key('building-search-error'),
                        child: Padding(
                          padding: const EdgeInsets.symmetric(
                            horizontal: MqSpacing.space8,
                          ),
                          child: Text(
                            l10n.buildingsLoadError,
