  ...
  group('OpenDayTime', () {
    test(
      ...
      () {
        ...
        final instant = DateTime.parse('2026-08-15T13:00:00+10:00');
        expect(OpenDayTime.formatTimeOfDay(instant), '1:00 PM');
      },
    );

    test('formats AEST 10:00 as 10:00 AM (boundary check)', () {
      final instant = DateTime.parse('2026-08-15T10:00:00+10:00');
      expect(OpenDayTime.formatTimeOfDay(instant), '10:00 AM');
    });

    test('formats time range with consistent AM/PM markers', () {
      final start = DateTime.parse('2026-08-15T13:00:00+10:00');
      final end = DateTime.parse('2026-08-15T13:30:00+10:00');
    ...
    test('long date renders as Saturday in Sydney for the 2026 cycle', () {
      ...
      final date = DateTime.parse('2026-08-15T10:00:00+10:00');
    ...
    test('sydneyHour returns wall-clock Sydney hour', () {
      ...
      // the Sydney hour.
      final asAEST = DateTime.parse('2026-08-15T13:00:00+10:00');
      final asUtc = DateTime.parse('2026-08-15T03:00:00Z'); // same moment
