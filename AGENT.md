...
### Raouf: 2026-08-04 (Australia/Sydney) — Fix Xcode "build failed"; make iOS signing per-developer so any machine can install
**Scope:** iOS code-signing configuration only — `ios/Runner.xcodeproj/project.pbxproj`, new `ios/Flutter/Signing.xcconfig`, `.gitignore`. No Dart, no app behaviour, no bundle-id change.
**Summary:**
- **Root cause.** Xcode reported a build failure while *every* Flutter CLI path built cleanly, which localised the fault to a target the CLI's default `build` action never touches. `Runner` was correctly signed with team `3L5A4R7JNY` ("Mohammad Raouf Abedini" — the only signing identity on this machine, `Apple Development: titanfall1380@gmail.com`, wildcard profile valid to 2027-02-25). `RunnerTests` was pinned to a **different** team, `94273WB4G3`, for which no certificate or private key exists locally. So any Xcode action that builds the test bundle — ⌘U, "Build for Testing", or a Test-action-inclusive build — failed at the signing step with `No "iOS Development" signing certificate matching team ID "94273WB4G3" with a private key was found`, taking the whole build red.
- **Durable fix, not just a re-point.** Simply changing the hardcoded id would have re-broken the moment a second developer built the project, so `DEVELOPMENT_TEAM` was removed from **all six** target-level build configurations (`Runner` ×3, `RunnerTests` ×3) and moved into a new `ios/Flutter/Signing.xcconfig`, attached as the **project-level** `baseConfigurationReference` on all three project configurations (Debug/Release/Profile). Project level is the key detail: `RunnerTests`' own base config is the CocoaPods-generated `Pods-RunnerTests.*.xcconfig`, which `pod install` regenerates and therefore cannot be edited, and the existing `Secrets.xcconfig` hook lives in `Flutter/Debug|Release.xcconfig` which only the `Runner` target consumes. Both targets now inherit one team from one file. Removal (rather than reassignment) is required because a value written in `project.pbxproj` **overrides** any xcconfig.
- **Per-developer override.** `Signing.xcconfig` ends with `#include? "Signing.local.xcconfig"` (optional include, Xcode 8+), following the convention the repo already uses for `Secrets.xcconfig`. A second developer adds a one-line gitignored `ios/Flutter/Signing.local.xcconfig` with their own `DEVELOPMENT_TEAM` and needs no other change; with no such file the committed default keeps working, so this machine and CI are unaffected. The bundle id is deliberately *not* made overridable — `ios/Runner/GoogleService-Info.plist` is issued against `com.pouya.mqnavigation`, so changing it would break Firebase messaging.
**Files Changed:** `ios/Runner.xcodeproj/project.pbxproj`, `ios/Flutter/Signing.xcconfig` (new), `.gitignore`, `AGENT.md`, `CHANGELOG.md`.
**Verification:** Reproduced and fixed against the exact failing command, device-targeted at the physical iPhone (`-destination id=00008150-000E7C6A1EF0401C`): `xcodebuild … build-for-testing` → **`** TEST BUILD FAILED **`** before, **`** TEST BUILD SUCCEEDED **`** after. Confirmed the app code was never at fault — `flutter build ios --debug --no-codesign`, `flutter build ios --debug` (signed) and `xcodebuild … build` all succeeded *before* any change. Override precedence proven directly via `-showBuildSettings`: default resolves `DEVELOPMENT_TEAM = 3L5A4R7JNY`; with a temporary `Signing.local.xcconfig` it resolves `ZZ9PLURAL9`; file removed afterwards (it is gitignored). `flutter build ios --debug` still reports `Automatically signing iOS for device deployment using specified development team in Xcode project: 3L5A4R7JNY`, i.e. Flutter's own signing path resolves the team from the xcconfig. `grep -c DEVELOPMENT_TEAM project.pbxproj` → **0** (also 0 after a subsequent `pod install`, confirming CocoaPods does not reinstate it); `grep -rn 94273WB4G3` → 0 repo-wide. `flutter analyze --no-fatal-infos` → 3 pre-existing infos (`anonKey` deprecation, 2 in `scene_rail_test.dart`). `flutter test` → **617/617 passed**. Release build installed and verified on the physical device: `xcrun devicectl device info apps` lists `MQ Journey / com.pouya.mqnavigation / 1.0.0`.
**Follow-ups:**
- **Xcode's Signing & Capabilities UI can undo this.** Toggling anything on that tab writes `DEVELOPMENT_TEAM` back into `project.pbxproj`, where it silently shadows the xcconfig again. Prefer editing `Signing.local.xcconfig`. A CI guard asserting `project.pbxproj` contains no `DEVELOPMENT_TEAM` would make a regression fail loudly — not added here.
- **Free Apple IDs issue 7-day provisioning profiles.** If the mate uses a free account, the installed app stops launching after a week and must be re-installed; a paid account (or being added to a shared paid team) is the only way around it. The current team `3L5A4R7JNY` has a profile valid to 2027-02-25.
- `RunnerTests` still carries the stale bundle id `io.mqnavigation.mqNavigation.RunnerTests` (and `Runner` is `com.pouya.mqnavigation`), both predating the `io.mqjourney` rename. Left untouched — renaming `Runner`'s bundle id invalidates the installed app, the Firebase plist and the Maestro `appId`. The CI stale-name guard greps for `mq_navigation` and so does not catch camel-cased `mqNavigation`.

### Raouf: 2026-07-27 (Australia/Sydney) — Open Day demo deck: MQ-Journey-OpenDay-Deck.pptx
**Scope:** Presentation asset only — no app code touched. New 12-slide deck at the repo root for the Open Day booth demonstration.
**Summary:** Generated a 12-slide 16:9 deck in the app's own Open Day identity (openDayMagenta `#C6006F` / openDayPlum `#7C1850` / alabaster, Georgia + Calibri) via pptxgenjs. Story arc: title → problem → scan flow → app tour (real screenshots from `screenshots/`) → six-feature grid → verified numbers (617/617 tests, 35 locales, 170 buildings, 77 sessions, 26 panoramas, 0 trackers) → privacy & CI guards → georeferencing deep dive (40m→28m median pin error) → 2026 stack (Flutter 3.44 Impeller/SPM defaults, Riverpod 3 auto-retry, Supabase anonymous auth — claims checked against current official docs) → 12-gate CI → booth demo script with the real Ed25519-signed `hadenfeld-10` QR rendered on-slide → close. The earlier `MQ-Journey-Pitch-Deck.pptx` (untracked, stale numbers: 542 tests / 161 buildings) was left untouched.
**Files Changed:** `MQ-Journey-OpenDay-Deck.pptx` (new), `AGENT.md`, `CHANGELOG.md`.
**Verification:** Two independent visual-QA passes over rendered slide images (fresh-eyes subagents) plus a targeted re-check; all found layout collisions fixed (title-chip/phone overlap, asymmetric grid margins, widows, vertical-centering of body text). markitdown content scan: 0 placeholder strings, key facts present. Note: `screenshots/QR code Scanner.png` captures the camera-permission-denied state, so slide 3 uses a printed-placard mock around the real signed QR instead.
**Follow-ups:** Reshoot the scanner screenshot with camera permission granted; README badges are stale (535 tests / Flutter 3.41+ vs actual 617 / 3.44.7) — worth refreshing before Open Day.

### Raouf: 2026-07-23 (Australia/Sydney) — Re-georeference the campus map: pins now match the drawing
**Scope:** `assets/data/buildings.json` (all 170 `campusX/campusY`), `assets/data/campus_overlay_meta.json` (affine coefficients + pixel offsets). Data + calibration only; no rendering-code change. Follows the earlier 2026-07-23 entry, which fixed the RTL nav and left this offset explicitly unresolved.
**Summary:**
- **Found real ground truth.** The previous entry concluded the ~57m pin/dot mismatch was irreducible scatter. That was wrong, and the reason was a missing reference: `assets/maps/mq-campus.png` carries the university's own **printed reference grid**, and 70 buildings already carry a `gridRef` naming their cell. That is a third, independent source — beholden to neither our GPS data nor our hand-placed pixels. Recovered the grid geometry directly off the raster by comb-autocorrelating its row/column brightness profiles: **31 columns of 141.5px from x=84.75, 22 rows (A..W, no I) of 142.5px from y=61.75**, i.e. square cells of exactly one 50m scale-bar division. Verified by overlaying the derived lines on the raster at both corners — every printed label sits centred in its predicted cell, with no drift across the full 4678×3307 sheet.
- **The map is affine-registerable after all; the old transform simply wasn't fitted to it.** Re-fitting GPS→pixel against the grid-cell centres by robust least squares (iterative outlier rejection) converges at a **93px median residual against an 82px quantisation floor** — i.e. the new transform explains the grid almost exactly to the limit a cell-level reference can resolve (true error ≈44px ≈ **16m**). The earlier "a quadratic barely beats an affine, therefore the map is distorted" inference was measuring against the *pins*, which are the noisy party; against the grid the affine is near-optimal.
- **Regenerated every pin, source-selected per building.** Where a `gridRef` exists, took whichever of {new-affine(GPS), existing hand pin} the map's own grid actually agrees with — 142 buildings resolved to GPS, **28 kept their hand pin** because their GPS is demonstrably wrong (e.g. `10HA` is 414m out, `WARATAH`, `NEXTSENSE`, `MQV`, `6ER`). Where no `gridRef` exists, used the validated transform on real GPS. Set `buildingPixelOffsetX`/`pixelOffsetX` to **0**: pins and the user-location dot now share one pixel space by construction, so the 80px (~29m) systematic gap between them is gone.
- **Fixed 9 fabricated records.** The Wally's Walk trail stops were not merely un-pinned at `(0,0)` — their *coordinates were invented*: a perfect arithmetic sequence stepping −0.0001 lat / +0.0001 lng per entry, a synthetic 45° line through campus. All 9 are duplicates of properly georeferenced buildings; 7 were resolved to their canonical record (`wallys-29`→`29WW`, `wallys-25`→`25WW`, `wallys-23`→`23WW`, `wallys-21`→`MQTH`, `wallys-17`→`17WW`, `ondaatje-14`→`14SCO`, `hadenfeld-10`→`10HA`), inheriting its GPS, `gridRef` and final pin — so a canonical entry with bad GPS can't poison its duplicate.
**Files Changed:** `assets/data/buildings.json`, `assets/data/campus_overlay_meta.json`, `test/features/map/building_registry_asset_test.dart` (new grid guard; `buildingPixelOffsetX` assertion inverted), `AGENT.md`, `CHANGELOG.md`.
**Verification:** Agreement with the map's printed grid, measured before → after: **exact-cell 38% → 54%**, median offset **111px (40m) → 77px (28m)**, p90 273px → 213px. 77px is at the ~82px floor that cell-quantised references impose, so this is close to the best this ground truth can certify. Independently spot-checked by rendering the regenerated pins onto the raster and reading them against the printed building labels — SEC, HOSP, AINS, LIB, SPORT, LOTUS, PRICE, MQTH, 25BWW, COCHLEAR, GUMNUT, WARATAH, INCUB, OBS, MQV, CLINIC, 75TAL and the whole Wally's Walk run now land on their named buildings. New regression test `building pins agree with the campus map printed grid` confirmed **failing on the pre-change data** (median 137px vs the <100px bound) and passing after. `flutter analyze --no-fatal-infos` → 3 pre-existing infos. `flutter test` → **617/617 passed**.
**Follow-ups:**
- **Bad source GPS, not fixed here — these buildings will route wrongly.** `10HA` (414m out), `WARATAH`, `NEXTSENSE`, `NEXTSCHOOL`, `MQV`, `6ER`, `8SCO`, `14SCO` all have coordinates that contradict the map grid; their *pins* are now correct (hand pin retained) but their lat/lng still needs re-surveying.
- **Still unresolved after the sweep:** `RMC`, `13RPD`, `13ARPD`, `14ER` — neither GPS nor pin agrees with the stated `gridRef`, so one of the three is wrong and a human needs to adjudicate. `wallys-27` and `wallys-1` have **no canonical building** in the registry (no `27WW`/`1WW`), so their pins were left untouched and remain unplaced.
- The grid constants are duplicated between the regression test and the (throwaway) calibration script. If the map raster is ever replaced, re-derive them — the test will fail loudly first.
- `campusX/campusY` is now redundant for the 142 GPS-sourced buildings (it is exactly `affine(GPS)`). A later cleanup could drop the field and project everything at runtime, leaving only the ~28 genuine hand-placed overrides.

### Raouf: 2026-07-23 (Australia/Sydney) — Fix RTL bottom-nav mirroring + campus-map location accuracy
**Scope:** `LiquidTabBar` behaviour under RTL locales (fa/ar/he/ur); campus-overlay building pinning; Android GPS provider selection. Also bumped the pinned Flutter SDK.
**Summary:**
- **RTL bottom nav (the "wrong animation" in Persian).** The tab strip is a plain `Row`, so under `TextDirection.rtl` Flutter mirrors it and item 0 renders at the *physical right*. But both of the bar's coordinate consumers are physical-left-origin and do **not** mirror: hit-testing used `d.localPosition.dx` directly, and the lens indicator used `Positioned(left: (frac + 0.5) * slot)` (`Positioned.left` is absolute — only `PositionedDirectional` flips). Net effect in Persian: **every tab tap navigated to the mirrored tab** (tapping Scan opened Home) and the lens glided to the opposite end of the bar. Fixed by flipping between index-space and physical-pixel space once, in `build`: pointer x is converted with `localDx(p) => rtl ? w - p.dx : p.dx` before `_indexAt`/`_fracAt`, and the indicator uses `visualFrac = rtl ? (n - 1) - frac : frac`. LTR maths is byte-for-byte unchanged.
- **Buildings pinned to the map's top-left corner.** `campusX: 0, campusY: 0` is the "not yet digitised" sentinel in `assets/data/buildings.json`, but `hasCampusCoordinates` only null-checked, so `campusPoint` returned a literal pixel `(0,0)` → `buildingPixelToMapPoint` → the top-left corner of the 4678×3307 overlay raster. **9 buildings** were affected — all of them the recently shipped Wally's Walk trail stops plus 10 Hadenfeld Ave and 14 Ondaatje Ave (`wallys-1/17/21/23/25/27/29`, `hadenfeld-10`, `ondaatje-14`). They now fail the check and fall through to the existing GPS-projection path in `resolveBuildingPoint`, landing at their real position. A zero on a *single* axis is still honoured as a legitimate edge coordinate.
- **GPS drift/jitter on Android.** `AndroidSettings.forceLocationManager: true` (in both `getCurrentLocation` and `watch`) bypassed the Play-Services Fused Location Provider for the raw OS `LocationManager`. The original rationale was avoiding Wi-Fi triangulation, but raw GPS has no sensor/Wi-Fi fusion and no smoothing, so between the buildings along Wally's Walk the dot drifted and jumped tens of metres per update and often never locked indoors. Removed the flag on both call sites; `bestForNavigation` on the fused provider is both more accurate and far more stable in a dense low-rise campus.
- **Toolchain.** `pubspec.yaml` pins an exact Flutter version and CI installs from it (`flutter-version-file: pubspec.yaml`). Local stable is now 3.44.7 and the exact `"3.44.0"` pin made `pub get` unsolvable; bumped the pin to `"3.44.7"`.
**Files Changed:** `lib/app/router/liquid_tab_bar.dart`, `lib/features/map/domain/entities/building.dart`, `lib/features/map/data/datasources/location_source.dart`, `pubspec.yaml`, `test/app/router/liquid_tab_bar_test.dart` (2 new RTL regression tests), `test/features/map/building_test.dart` (2 new sentinel tests), `AGENT.md`, `CHANGELOG.md`.
**Verification:** All four new tests confirmed **failing before** the fix and passing after — the RTL tap test recorded `[0]` instead of `[1]` (the mirrored tab), the lens test measured the indicator centre at x=200 on a 800px bar instead of >400, and the sentinel test returned `hasCampusCoordinates == true` for `(0,0)`. `flutter analyze --no-fatal-infos` → 3 issues, all pre-existing (`anonKey` deprecation, two in `scene_rail_test.dart`). `flutter test` → **616/616 passed**. `dart format` applied.
**Follow-ups:**
- **The remaining campus-map offset is data debt, not a code bug — not fixed here.** Measured over the 126 buildings that carry both GPS and `campusX/campusY`: the shipped `gcp_affine` transform lands a median **159px (≈57m)** away from the building's own pin. Refitting the affine against those pins by robust least squares only gets the median to **98px (≈35m)**, and going to a 6-param quadratic barely moves it (132px → 130px). That non-convergence means the residual is *scatter in the hand-placed pin coordinates*, not a smooth distortion any global transform can absorb — so the dot and the pins will keep disagreeing by ~35m until the pins are re-digitised against `assets/maps/mq-campus.png`. Worst individual offenders: `RONREILLYP` (1726px), `FIELDS` (1555px), `10HA` (953px), `BUMQ` (946px), `CULT` (701px), `WARATAH` (658px). Note `campus_overlay_meta.json` + `buildingPixelOffsetX: 80` come from the shared web export (see 2026-06 entry), so re-digitising should happen there and re-export, not fork here.
- Device-verify the Android fused-provider change on a physical handset on campus — the accuracy claim is reasoned from the provider semantics, not measured in situ.
- The 9 formerly-corner-pinned buildings now render from GPS; give them real `campusX/campusY` so they sit on the drawn buildings.
